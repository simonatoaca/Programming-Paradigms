...
(define (num->base n b)
  (if (zero? n)
      null
      (append (num->base (quotient n b) b) (list (remainder n b)))))
...
(define (palindrome-in-b-bases? n b [max-base 10])
  (or (zero? b)
      (and (>= max-base 2)
          (if (palindrome? (num->base n max-base))
              (palindrome-in-b-bases? n (- b 1) (- max-base 1))
              (palindrome-in-b-bases? n b (- max-base 1))))))

(define (first-b-pal start b)
  (if (palindrome-in-b-bases? (+ start 1) b)
      (+ start 1)
...
; transforma un nr intr o lista de cifre (in ordine inversa)
(define (num->list n)
  (if (zero? n)
      null
      (cons (remainder n 10) (num->list (quotient n 10)))))

; determina palindromul maxim din lista incepand de pe poz 0
(define (max-palindrome-in-list L n)
  (if (palindrome? (take L n))
      (take L n)
      (max-palindrome-in-list L (- n 1))))

; verifica palindroame de pe toate pozitiile listei si il alege pe cel mai mare
(define (longest-palindrome-list L prev-list prev-list-size)
  (if (null? L)
      prev-list
      (if (> (length (max-palindrome-in-list L (length L))) prev-list-size)
          (longest-palindrome-list (cdr L) (max-palindrome-in-list L (length L)) (length (max-palindrome-in-list L (length L))))
          (longest-palindrome-list (cdr L) prev-list prev-list-size))))

; din lista palindrom -> inapoi in numar
(define (longest-palindrome n)
  (list->num (longest-palindrome-list (num->list n) null 0)))
