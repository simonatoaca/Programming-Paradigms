...
myConcat([], L, L) :- true.
myConcat([H | Rest], L, [H | L1]) :- myConcat(Rest, L, L1).
...
myReverse(_,_):- false.
myReverse([], []) :- true.
myReverse([H | T], RL) :- myReverse(T, RevT), myConcat(RevT, [H], RL). 
...
myReverseAcc([H | T], Acc, L) :- myReverseAcc(T, [H | Acc], L).
...
isLeaf(Node):- nod(Node), \+ arc(Node, _).
...
isRoot(Node):- nod(Node), \+ arc(_, Node).
...
descendantOf(X,Y):- arc(Y, X); arc(Desc, X), descendantOf(Desc, Y).
...
sameTree(Nod1, Nod2):- descendantOf(Nod1, Nod2);
			descendantOf(Nod2, Nod1); descendantOf(Nod1, X), descendantOf(Nod2, X).
...
drum(Nod1, Nod2, Drum):- Nod1 = Nod2, Drum = Nod1; arc(Nod1, Nod2), Drum = [Nod1, Nod2];
					arc(Nod2, Nod1), Drum = [Nod2, Nod1]; descendantOf(X, Nod1), descendantOf(Nod2, X),
					drum(X, Nod2, T), Drum = [Nod1 | T]; descendantOf(Nod1, X), drum(X, Nod2, T), Drum = [Nod1 | T].
