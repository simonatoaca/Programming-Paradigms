...
instance Invertible (BST a) where
    ...
    invert tree = BSTNod { vl = vl tree, lt = invert (rt tree), rt = invert (lt tree)}
...
instance Functor BST where
    ...
    fmap f tree = BSTNod {vl = f (vl tree), lt = (fmap f (lt tree)), rt = (fmap f (rt tree))}
...
instance Foldable BST where
    ...
    foldr f acc tree = foldr f (f (vl tree) (foldr f acc (rt tree))) (lt tree)
...
instance Container BST where
    contents tree = foldr (\x acc -> x : acc) [] tree
