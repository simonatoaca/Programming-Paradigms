...
removeDuplicatesLeft l = reverseList (foldl (\acc x -> if elem x acc then acc else x : acc) [] l)
...
mergeSort lst =
    ...
    in (if null lst then [] else
        if null (tail lst) then lst else merge (mergeSort (fstHalf lst)) (mergeSort (sndHalf lst)))

mergeSort2 :: [Int] -> [Int]
mergeSort2 lst
        | null lst = []
        | null (tail lst) = lst
        | otherwise = merge (mergeSort (fstHalf lst)) (mergeSort (sndHalf lst))
    where 
        computeHalf = \f lst -> f (quot (length lst) 2) lst
        fstHalf = \lst -> computeHalf take lst
        sndHalf = \lst -> computeHalf drop lst
...
playerWins :: Int -> Bool
playerWins candies =
    let player = \candies -> (candies > 0) && ((not (opponent (candies - 1))) || 
                                               (not (opponent (candies - 2))))
        opponent = \candies -> (candies > 1) && ((not (opponent (candies - 2))) ||
                                                 (not (opponent (candies - 3))))
    in (player candies)

playerWins2 :: Int -> Bool
playerWins2 candies = player candies
    where 
        player = \candies -> (candies > 0) && ((not (opponent (candies - 1))) || 
                                               (not (opponent (candies - 2))))
        opponent = \candies -> (candies > 1) && ((not (opponent (candies - 2))) ||
                                                 (not (opponent (candies - 3))))
