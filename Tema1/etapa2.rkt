...
(define (get-pref-list pref person)
  (cdar (filter (λ (x) (equal? (car x) person)) pref)))
...
(define (preferable? pref-list x y)
  (and (member y (member x pref-list)) ; daca y se afla in lista incepand cu elem x -> true
       #t))
...
(define (get-partner engagements person)
  (define x (find-first (λ (x) (equal? (car x) person)) engagements))
  (and x (cdr x)))
